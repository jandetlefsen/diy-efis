...
#define FTL_ERR_VERSUNKNOWN   (FTL_ERR_BASE + 3)
#define FTL_ERR_VERSMISMATCH  (FTL_ERR_BASE + 4)
...
#define FTL_ERR_CACHE_ALLOCTARRAY                             (FTL_ERR_CACHE_BASE + 42)
...
#define AUTOFILL_0xFF_UNMAPPED_SECTORS true

// user configurable parameter
#define CACHE_RAM_BD_MODULE            false
#if (CACHE_RAM_BD_MODULE == true)
#define CACHE_MINIMUM_RAM           false
#define CACHE_MAXIMUM_RAM           false // debug only.
#if ((CACHE_MINIMUM_RAM == true) && (CACHE_MAXIMUM_RAM == true))
...
#define CACHE_DYNAMIC_ALLOCATION    false

#if (CACHE_DYNAMIC_ALLOCATION == true)
uint32_t gTotal_ram_allowed;
#endif

#endif

//.System Configuration/Feature  Flags
#define FTL_BACKGROUND_ERASE        false
#define FTL_RPB_CACHE               false
#define FTL_RPB_CACHE_API_PFT_SAFE  true
#define FTL_TABLE_LOCATION          FTL_TABLE_RAM
#define FTL_PERFORMANCE_MARKERS     true
#define FTL_ENABLE_DELETE_API       true
#define FTL_DELETE_WITH_GC          true
#define FTL_EBLOCK_CHAINING         true
#if (CACHE_RAM_BD_MODULE == true)
#define FTL_ENABLE_UNUSED_EB_SWAP   false
#else
#define FTL_ENABLE_UNUSED_EB_SWAP   true
#endif
#define FTL_CHECK_ERRORS            true
#define FTL_STATIC_WEAR_LEVELING    true
#define FTL_CHECK_VERSION           true
#define FTL_NON_ALIGNED_BUFFER      true
#define FTL_UNLINK_GC               true
#if (FTL_SUPER_SYSEB_MODE == FTL_SUPER_SYSEB_ENABLE)
#define FTL_SUPER_SYS_EBLOCK        true
#else
#define FTL_SUPER_SYS_EBLOCK        false
#endif  // #if (FTL_SUPER_SYSEB_MODE == FTL_SUPER_SYSEB_ENABLE)
#define INVERT_FLASH_MMAP           false
#define MAX_BAD_BLOCK_SANITY_TRIES  4

#if (FTL_DEVICE_TYPE == FTL_DEVICE_NAND)
#define FTL_REDUNDANT_LOG_ENTRIES   false
#define FTL_DEFECT_MANAGEMENT       true
#else
#define FTL_REDUNDANT_LOG_ENTRIES   true
#define FTL_DEFECT_MANAGEMENT       false
#endif

#if (FTL_DEVICE_TYPE == FTL_DEVICE_NAND)
#define FTL_CHECK_BAD_BLOCK_LIMIT     false
#define FTL_ENABLE_NAND_SECOND_CHIP   false
#endif

// three options for logging if FTL_DEVICE_TYPE is set to FTL_DEVICE_NOR
// 1- use only A half - FTL_REDUNDANT_LOG_ENTRIES = false
//    ftl_def.h (LOG_ENTRY_DELTA = 32)
// 2- redundant copy - FTL_REDUNDANT_LOG_ENTRIES = true
//    ftl_def.h (LOG_ENTRY_DELTA = 32)
// 3- use both A and B half - FTL_REDUNDANT_LOG_ENTRIES = false
//    ftl_def.h (LOG_ENTRY_DELTA = 16)
#if (FTL_STATIC_WEAR_LEVELING == true)
#define STATIC_WL_THRESHOLD_COUNT  200
#endif

#define FTL_ENABLE_DEBUG_CODE       false  /*enables FTL DEBUG CODE*/

// FTL_ENABLE_DEBUG_CODE is not supported with CACHE_RAM_BD_MODULE.
#if ((FTL_ENABLE_DEBUG_CODE == true) && (CACHE_RAM_BD_MODULE == false))
#define DEBUG_COMPARE_TABLES        false
...
#define FTL_DEBUG_STRUCT_SIZE       true
#else
#define FTL_DEBUG_STRUCT_SIZE       false
#endif
#define DEBUG_INIT_CHECK            true
#define FTL_DEBUG_FLASH_USAGE       true

#else  // #if (FTL_ENABLE_DEBUG_CODE == true)
#define DEBUG_COMPARE_TABLES        false
#define FTL_DEBUG_STRUCT_SIZE       false
#define DEBUG_INIT_CHECK            false
#define DEBUG_ENABLE_LOGGING        false
#define FTL_DEBUG_FLASH_USAGE       false

#endif  // #else  // #if (FTL_ENABLE_DEBUG_CODE == true)

#if (FTL_ENABLE_UNUSED_EB_SWAP == true)
#define ENABLE_EB_ERASED_BIT       (true)

#else  // #if (FTL_ENABLE_UNUSED_EB_SWAP == true)
#define ENABLE_EB_ERASED_BIT       (false)
#endif  // #else  // #if (FTL_ENABLE_UNUSED_EB_SWAP == true)
...
typedef struct _transMap
  ...
  uint8_t startSector; /*1*/
  uint32_t startLBA; /*4*/
  uint32_t phyPageAddr; /*4*/
  uint32_t mergePageForWrite; /*4*/
  uint16_t logEBlockEntryIndex; /*4*/
...
typedef struct _ftlInitVars
  ...
  uint32_t total_ram_allowed;
#endif
  } FTL_INIT_STRUCT;

typedef struct _capacity
  {
  uint32_t totalSize; //In Bytes
  uint32_t numDBlocks;
  uint32_t numEBlocks;
  uint32_t eBlockSizeBytes;
  uint32_t eBlockSizePages;
  uint32_t pageSizeDBlocks;
  uint32_t pageSizeBytes;
  uint32_t numBlocks; // is equivalent to numDBlocks OBSOLETE
  uint32_t blockSize; // is equivalent to numEBlocks OBSOLETE
  uint32_t pageSize; // is equivalent to pageSizeBytes OBSOLETE
  uint32_t busWidth;
  uint32_t numDevices;
  } FTL_CAPACITY;
...
typedef struct _devConfig
  {
  uint16_t dataEBlkStart;
  uint16_t dataEBlkEnd;
  uint16_t reserveEBlkStart;
  uint16_t reserveEBlkEnd;
  uint16_t sysEBlkStart;
  uint16_t sysEBlkEnd;
  uint16_t numDevices;
  uint16_t sectorsPerPage;
  uint16_t dataPagesPerEBlock;
  uint16_t sysPagesPerEBlock;
  uint16_t eblockChainingEnabled;
  } FTL_DEV_CONFIG;
...
extern "C"
  ...
  FTL_STATUS FTL_SetXtremeModeProgramming(uint8_t mode);
  FTL_STATUS FTL_GetUtilizationInfo(uint32_t DeviceID, uint32_t * Free, uint32_t * Used,
                                    uint32_t * Dirty);
  FTL_STATUS FTL_GetEraseCycles(uint32_t DeviceID, uint32_t * high_block_number,
                                uint32_t * high_erase_count, uint32_t * low_block_number, uint32_t * low_erase_count);
  /* Cache and Tanslation Table */
  FTL_STATUS FTL_FlushDataCache(void);
  FTL_STATUS FTL_FlushTableCache(void);
  /* Read */
  FTL_STATUS FTL_DeviceObjectsRead(uint8_t * buffer, uint32_t LBA, uint32_t NB,
                                   uint32_t * bytesDone);
  /* Write */
  FTL_STATUS FTL_DeviceObjectsWrite(uint8_t * buffer, uint32_t LBA, uint32_t NB,
                                    uint32_t * bytesDone);
  /* Delete */
  FTL_STATUS FTL_DeviceObjectsDelete(uint32_t LBA, uint32_t NB, uint32_t * sectorsDeleted);
  /* Garbage Collection */
  FTL_STATUS FTL_ForcedGC(FTL_DEV DeviceID, uint16_t logicalEBNum, uint16_t * FreedUpPages,
                          uint16_t * freePageIndex);
  FTL_STATUS FTL_SetGCThreshold(uint8_t type, uint32_t threshold);
  /*Trafer map*/
  FTL_STATUS FTL_BuildTransferMapForWriteBlocking(uint32_t LBA,
                                                  uint32_t NB, uint32_t * resultingEntries);
  FTL_STATUS FTL_BuildTransferMapForReadBlocking(uint32_t LBA,
                                                 uint32_t NB, uint32_t * resultingEntries);
  FTL_STATUS FTL_GetTransferMapEntry(uint16_t entryIndex, TRANS_MAP_ENTRY_PTR transferMapEntry);
  FTL_STATUS FTL_TransferPageForWrite(uint8_t **byteBuffer, uint32_t *remainingEntries);
  ...
  FTL_STATUS TST_GetLogicalEBEraseCount(uint8_t devID, uint16_t logicalEBNum,
                                        uint16_t* physicalEBNum, uint32_t* eraseCount);
  FTL_STATUS TST_GetPhysicalEBEraseCount(uint8_t devID, uint16_t phyEBNum, uint32_t* eraseCount);
  FTL_STATUS TST_GetAverageEraseCount(uint8_t devID, uint16_t Start_Logical_eBlk,
                                      uint16_t End_Logical_eBlk, uint16_t* avgEraseCount);
  FTL_STATUS TST_CheckEraseCount(uint8_t devID, uint16_t Start_Logical_eBlk,
                                 uint16_t End_Logical_eBlk, uint16_t expected_value);
