...
uint8_t EBlockMappingCache[NUM_EBLOCK_MAP_INDEX * NUMBER_OF_DEVICES][FLUSH_RAM_TABLE_SIZE];
uint8_t PPAMappingCache[NUM_PPA_MAP_INDEX * NUMBER_OF_DEVICES][FLUSH_RAM_TABLE_SIZE];
uint16_t EBMCacheIndex[NUM_EBLOCK_MAP_INDEX * NUMBER_OF_DEVICES];
#else
uint32_t gSaveTotalRamAllowed = 0;
uint8_t gCheckFirstMalloc = false;
uint8_t ** EBlockMappingCache;
uint8_t ** PPAMappingCache;
uint16_t * EBMCacheIndex;

uint16_t numBlockMapIndex = 0;
uint16_t numPpaMapIndex = 0;
uint16_t cacheIndexChangeArea = 0;
uint16_t thesholdDirtyCount = 0;
uint16_t ebmCacheIndexSize = 0;
uint16_t eblockMappingCacheSize = 0;
uint16_t ppaMappingCacheSize = 0;
#endif // #if (CACHE_DYNAMIC_ALLOCATION == false)
uint16_t EBlockMapIndex[MAX_EBLOCK_MAP_INDEX * NUMBER_OF_DEVICES];
uint16_t PPAMapIndex[MAX_PPA_MAP_INDEX * NUMBER_OF_DEVICES];
uint32_t RamMapIndex[NUMBER_OF_ERASE_BLOCKS * NUMBER_OF_DEVICES];
#else
...
uint8_t PPAMappingTableDirtyBitMap[NUM_DEVICES][PPA_DIRTY_BITMAP_DEV_TABLE_SIZE];
uint8_t EBlockMappingTableDirtyBitMap[NUM_DEVICES][EBLOCK_DIRTY_BITMAP_DEV_TABLE_SIZE];
#endif
KEY_TABLE_ENTRY FlushLogEBArray[NUM_DEVICES][NUM_FLUSH_LOG_EBLOCKS];
KEY_TABLE_ENTRY TransLogEBArray[NUM_DEVICES][NUM_TRANSACTLOG_EBLOCKS];
uint16_t FlushLogEBArrayCount[NUM_DEVICES];
uint16_t TransLogEBArrayCount[NUM_DEVICES];
uint32_t TransLogEBCounter[NUM_DEVICES];
uint32_t FlushLogEBCounter[NUM_DEVICES];
#if (FTL_SUPER_SYS_EBLOCK == true)
KEY_TABLE_ENTRY SuperSysEBArray[NUM_DEVICES][NUM_SUPER_SYS_EBLOCKS];
uint16_t SuperSysEBArrayCount[NUM_DEVICES];
uint32_t SuperSysEBCounter[NUM_DEVICES];
SUPER_EB_INFO SuperEBInfo[NUM_DEVICES];
uint8_t gProtectForSuperSysEBFlag;
#if(FTL_DEVICE_TYPE == FTL_DEVICE_NAND)
uint8_t packedSuperInfo[SECTOR_SIZE];
#endif
#endif  // #if (FTL_SUPER_SYS_EBLOCK == true)
uint32_t GCNum[NUM_DEVICES];
GC_INFO GC_Info;
uint16_t GC_THRESHOLD;
TRANSFER_MAP_STRUCT transferMap[NUM_TRANSFER_MAP_ENTRIES];
uint16_t TransferMapIndexEnd;
uint16_t TransferMapIndexStart;
FTL_DEV previousDevice;
uint8_t GCMoveArray[NUM_PAGES_PER_EBLOCK];
uint8_t GCMoveArrayNotEmpty;

#if(FTL_DEVICE_TYPE == FTL_DEVICE_NOR)
TRANS_LOG_ENTRY TransLogEntry;
uint16_t TranslogBEntries;
#endif  // #if(FTL_DEVICE_TYPE == FTL_DEVICE_NOR)

uint8_t pseudoRPB[NUM_DEVICES][NUMBER_OF_BYTES_PER_PAGE];
uint32_t LastTransLogLba;
uint8_t LastTransLogNpages;
uint8_t Delete_GC_Threshold;
uint8_t FTL_initFlag = INIT_NOT_DONE;
uint8_t FTL_UpdatedFlag = UPDATED_NOT_DONE;
DEL_INFO Del_Info;
uint16_t ReservedEBlock[NUM_DEVICES][NUMBER_OF_SYSTEM_EBLOCKS];
uint16_t ReservedEBlockNum[NUM_DEVICES]; /* changed by Nobu Feb 18, 2015 : uint8_t -> uint16_t for ML16G2 */
uint32_t FlushEBlockAdjustedFreePage[NUM_DEVICES];
...
#if (CACHE_RAM_BD_MODULE == true)
uint8_t gCounterLRU;
uint16_t gCounterDirty;
uint16_t gDataAreaCounterDirty;
uint8_t gCacheNum;
uint16_t gCrossedLEB[NUM_CROSS_LEB];
#if(FTL_DEVICE_TYPE == FTL_DEVICE_NAND)
uint8_t gCheckPfForNand = false;
uint16_t gTargetPftEBForNand = EMPTY_WORD;
#endif

// test structure
#ifdef DEBUG_TEST_ARRAY
uint32_t EBlockMappingTableTest[NUMBER_OF_ERASE_BLOCKS][EBLOCK_MAPPING_ENTRY_SIZE / 4];
uint32_t PPAMappingTableTest[NUMBER_OF_ERASE_BLOCKS][(NUMBER_OF_PAGES_PER_EBLOCK * PPA_MAPPING_ENTRY_SIZE) / 4];
#endif
...
uint8_t gTestFlag;
uint8_t arrayOffsetTemp;
uint16_t glogicalEBtemp;
#endif

#ifdef DEBUG_CROSS_BOUNDARY_ROUTE
uint32_t gCacheHit = 0;
uint32_t gCacheMiss = 0;
uint32_t gNoUpAndDownCase = 0;
uint32_t gNoDownCase = 0;
uint32_t gNoUpCase = 0;
uint32_t gDistanceUpAndDownCase = 0;
#endif
#endif

#if(FTL_CHECK_BAD_BLOCK_LIMIT == true)
uint16_t gBBCount[NUMBER_OF_DEVICES];
uint16_t gBBDevLimit[NUMBER_OF_DEVICES];
#endif
...
void ClearTransLogEBBadBlockInfo(void)
  {
  badBlockInfo.TransLogEBFailed = false;
...
void SetTransLogEBFailedBadBlockInfo(void)
  {
  badBlockInfo.TransLogEBFailed = true;
  }
...
uint16_t GetLogicalEBlockAddr(FTL_DEV devID, uint16_t physicalBlockNum)
  {
  uint16_t logicalBlockNum = EMPTY_WORD;
#if (CACHE_RAM_BD_MODULE == true)
  uint32_t EBMramStructPtr = 0;
  uint32_t PPAramStructPtr = 0;
#endif
...
uint32_t GetEraseCount(FTL_DEV devID, uint16_t logicalBlockNum)
  ...
  eraseCount |= (uint32_t) (((*(EBLOCK_MAPPING_ENTRY_PTR) (EBMramStructPtr)).dirtyCount & ERASE_COUNT_GET_WORD_DIRTY_MASK) << ERASE_COUNT_DWORD_DIRTY_SHIFT);
  ...
  eraseCount |= (uint32_t) ((EBlockMappingTable[devID][logicalBlockNum].dirtyCount & ERASE_COUNT_GET_WORD_DIRTY_MASK) << ERASE_COUNT_DWORD_DIRTY_SHIFT);
...
#if (CACHE_RAM_BD_MODULE == true)
  CACHE_GetRAMOffsetEB(devID, logicalBlockNum, &EBMramStructPtr, &PPAramStructPtr);
  (*(EBLOCK_MAPPING_ENTRY_PTR) (EBMramStructPtr)).eraseCount = (uint16_t) (eraseCount & ERASE_STATUS_CLEAR_WORD_MASK);
...
#else
  EBlockMappingTable[devID][logicalBlockNum].eraseCount = (uint16_t) (eraseCount & ERASE_STATUS_CLEAR_WORD_MASK);
  if(ERASE_STATUS_GET_DWORD_MASK == (eraseCount & ERASE_STATUS_GET_DWORD_MASK))
    {
...
#endif
  return (uint16_t) (temp >> NUM_BITS_EB_CHAIN);
  }
...
uint16_t GetChainPhyEBNum(FTL_DEV devID, uint16_t logicalBlockNum)
  ...
  temp = (EBlockMappingTable[devID][logicalBlockNum].chainToFrom);
#endif
  return (uint16_t) (temp & EB_PHYSICAL_CHAIN_MASK);
  }
...
void SetChainLogicalEBNum(FTL_DEV devID, uint16_t logicalBlockNum, uint16_t logEBNum)
  {
  uint32_t temp = 0; /*4*/
  uint32_t temp2 = (uint32_t) logEBNum; /*4*/
...
FTL_STATUS UnlinkChain(FTL_DEV devID, uint16_t logicalBlockNumFrom, uint16_t logicalBlockNumTo)
  {
  uint16_t physicalBlockNumFrom = EMPTY_WORD; /*2*/
  uint16_t physicalBlockNumTo = EMPTY_WORD; /*2*/
  uint32_t eraseCountFrom = 0; /*2*/
  uint32_t eraseCountTo = 0; /*2*/
...
uint16_t GetChainWithLowestVaildPages(FTL_DEV devID)
  ...
  uint16_t numValidPages = 0; /*2*/
  uint16_t numValidLowest = EMPTY_WORD; /*2*/
...
uint16_t GetChainWithLowestVaildUsedRatio(FTL_DEV devID)
  ...
  uint16_t logicalEBNum = EMPTY_WORD; /*2*/
  uint16_t chainEB = 0; /*2*/
  uint32_t numUsed = 0; /*4*/
  uint16_t numUsedTemp = 0; /*2*/
  uint16_t numValidTemp = 0; /*2*/
  uint32_t numValid = 0; /*4*/
  uint32_t currentInverseRatio = 0; /*4*/
  uint32_t highestInverseRatio = 0; /*using inverse because interger division will produce 0*/
  uint16_t highEB = EMPTY_WORD; /*2*/
  uint8_t eblockCount = 0; /*1*/
  ...
  for(logicalEBNum = NUM_DATA_EBLOCKS; logicalEBNum < NUM_EBLOCKS_PER_DEVICE; logicalEBNum++)
    ...
    if(chainEB != EMPTY_WORD)
      ...
      numValid = (uint32_t) numValidTemp;
      numUsed = (uint32_t) numUsedTemp;
      if(numUsed == 0)
        {
        continue;
        }
      ...
      for(count = 0; count < NUM_CHAIN_EBLOCKS; count++)
        ...
        if(chainEB == SaveValidUsedPage[count + (devID * NUM_CHAIN_EBLOCKS)].LogEBNum)
          {
          numValid += (uint32_t) SaveValidUsedPage[count + (devID * NUM_CHAIN_EBLOCKS)].ValidPageCount;
          numUsed += (uint32_t) SaveValidUsedPage[count + (devID * NUM_CHAIN_EBLOCKS)].UsedPageCount;
          break;
          }
        }
#else
...
uint16_t GetNumValidPages(FTL_DEV devID, uint16_t logicalEBNum)
  ...
  for(temp = 0; temp < NUM_PAGES_PER_EBLOCK; temp++)
    {
#if (CACHE_RAM_BD_MODULE == true)
    bitValue = (FREE_BIT_MAP_TYPE) GetBitMapField((uint8_t *) (&((*(EBLOCK_MAPPING_ENTRY_PTR) (EBMramStructPtr)).freeBitMap[0])),
                                                  temp, FTL_BITS_PER_PAGE);
#else
    bitValue = (FREE_BIT_MAP_TYPE) GetBitMapField((uint8_t *) (&(EBlockMappingTable[devID][logicalEBNum].freeBitMap[0])),
                                                  temp, FTL_BITS_PER_PAGE);
#endif
...
void GetNumValidUsedPages(FTL_DEV devID, uint16_t logicalEBNum, uint16_t * used, uint16_t * valid)
  {
  uint16_t temp = 0; /*2*/
  uint16_t usedCount = 0; /*2*/
  uint16_t validCount = 0; /*2*/
  FREE_BIT_MAP_TYPE bitValue = 0; /*1*/
  ...
  if((logicalBlockNum < NUM_DATA_EBLOCKS) || (EMPTY_DWORD != EBlockMappingTable[devID][logicalBlockNum].chainToFrom))
    ...
    for(temp = 0; temp < NUM_PAGES_PER_EBLOCK; temp++)
      ...
      bitValue = (FREE_BIT_MAP_TYPE) GetBitMapField((uint8_t *) (&((*(EBLOCK_MAPPING_ENTRY_PTR) (EBMramStructPtr)).freeBitMap[0])),
                                                    temp, FTL_BITS_PER_PAGE);
#else
      bitValue = (FREE_BIT_MAP_TYPE) GetBitMapField((uint8_t *) (&(EBlockMappingTable[devID][logicalBlockNum].freeBitMap[0])),
                                                    temp, FTL_BITS_PER_PAGE);
#endif
      if(bitValue == BLOCK_INFO_EMPTY_PAGE)
        {
    ...
    {
#if (CACHE_RAM_BD_MODULE == true)
    return (uint16_t) ((*(EBLOCK_MAPPING_ENTRY_PTR) (EBMramStructPtr)).freePage_GCNum);
#else
    return (uint16_t) (EBlockMappingTable[devID][logicalBlockNum].freePage_GCNum);
#endif
    }
  return temp;
  }
...
#if (CACHE_RAM_BD_MODULE == true)
  SetBitMapField((uint8_t *) (&((*(EBLOCK_MAPPING_ENTRY_PTR) (EBMramStructPtr)).freeBitMap[0])),
...
#endif // #if(FTL_EBLOCK_CHAINING == true)
#else
  SetBitMapField((uint8_t *) (&(EBlockMappingTable[devID][logicalBlockNum].freeBitMap[0])),
                 usedPageIdx, FTL_BITS_PER_PAGE, BLOCK_INFO_VALID_PAGE);
...
#endif
#else
  sector = ((((uint32_t) logicalEBNum) * sizeof (EBLOCK_MAPPING_ENTRY)))
    / FLUSH_RAM_TABLE_SIZE;
  MarkEBlockMappingTableSectorDirty(devID, (uint16_t) sector);

  sector1 = ((((uint32_t) logicalEBNum + 1) * sizeof (EBLOCK_MAPPING_ENTRY)) - 1)
    / FLUSH_RAM_TABLE_SIZE;
  if(sector != sector1)
    {
    MarkEBlockMappingTableSectorDirty(devID, (uint16_t) sector1);
    }
#endif
  }
...
uint16_t GetPPASlot(FTL_DEV devID, uint16_t logicalEBNum, uint16_t logicalPageOffset)
  ...
  CACHE_GetRAMOffsetEB(devID, logicalEBNum, &EBMramStructPtr, &PPAramStructPtr);
  return (uint16_t) ((*(PPA_MAPPING_ENTRY*) (PPAramStructPtr + (PPA_MAPPING_ENTRY_SIZE * logicalPageOffset))) & EMPTY_INVALID);
#else
  return (uint16_t) (PPAMappingTable[devID][logicalEBNum][logicalPageOffset] & EMPTY_INVALID);
#endif
  }
...
#endif
#else
  sector = (((((uint32_t) logicalEBNum) * NUM_PAGES_PER_EBLOCK) + ((uint32_t) logicalPageOffset)) *
            sizeof (PPA_MAPPING_ENTRY)) / SECTOR_SIZE;
  MarkPPAMappingTableSectorDirty(devID, (uint16_t) sector);

#if(FTL_DEVICE_TYPE == FTL_DEVICE_NAND)
  sector1 = (((((uint32_t) logicalEBNum) * NUM_PAGES_PER_EBLOCK)) *
             sizeof (PPA_MAPPING_ENTRY)) / FLUSH_RAM_TABLE_SIZE;
  if(sector != sector1)
    {
    MarkPPAMappingTableSectorDirty(devID, (uint16_t) sector1);
    }
  sector1 = (((((uint32_t) logicalEBNum) * NUM_PAGES_PER_EBLOCK) + (NUM_PAGES_PER_EBLOCK - 1)) *
             sizeof (PPA_MAPPING_ENTRY)) / FLUSH_RAM_TABLE_SIZE;
  if(sector != sector1)
    {
    MarkPPAMappingTableSectorDirty(devID, (uint16_t) sector1);
...
void TABLE_ClearFreeBitMap(FTL_DEV devID, uint16_t eBlockNum)
  {
  uint16_t bitMapCounter = EMPTY_WORD; /*2*/
...
void TABLE_ClearMappingTable(FTL_DEV devID, uint16_t logicalEBNum, uint16_t phyEBAddr, uint32_t eraseCount)
  ...
  EBlockMappingTable[devID][logicalEBNum].eraseCount = (uint16_t) (eraseCount & ERASE_STATUS_CLEAR_WORD_MASK);
  if(ERASE_STATUS_GET_DWORD_MASK == (eraseCount & ERASE_STATUS_GET_DWORD_MASK))
    {
...
#endif
#if (FTL_DEFECT_MANAGEMENT == true)
  SetBadEBlockStatus(devID, logicalEBNum, false);
#endif
  }
...
FTL_STATUS TABLE_InitMappingTable(void)
  ...
  FTL_DEV devID = 0; /*1*/
  uint16_t eBlockNum = 0; /*2*/
#if (CACHE_RAM_BD_MODULE == false)
  uint16_t bitMapCounter = 0; /*2*/
#endif
...
FTL_STATUS FTL_ClearB(uint16_t count)
  ...
  TransLogEntry.entryB[count].seqNum = (uint8_t) (1 + count);
...
FTL_STATUS FTL_ClearC(uint16_t seqNum)
  ...
  TransLogEntry.entryC.seqNum = (uint8_t) (1 + seqNum);
...
FTL_STATUS TABLE_FlushEBGetNext(FTL_DEV devID, uint16_t * logicalAddrPtr, uint16_t * phyEBAddrPtr, uint32_t * keyPtr)
  {
  uint16_t eBlockNum = 0; /*2*/
  uint32_t smallestKey = EMPTY_DWORD; /*4*/
  uint16_t smallestKeyEB = EMPTY_WORD; /*2*/
...
FTL_STATUS TABLE_FlushEBGetLatest(FTL_DEV devID, uint16_t * flushEBlockPtr,
                                  uint16_t * phyEBAddrPtr, uint32_t key)
  {
  uint16_t block = 0; /*2*/
...
FTL_STATUS TABLE_GetFlushLogEntry(FTL_DEV devID, uint16_t blockNum, uint16_t * logicalEBNumPtr,
                                  uint16_t * phyAddrPtr, uint32_t * keyPtr)
  {
...
FTL_STATUS CACHE_DynamicAllocation(uint32_t totalRamAllowed)
  {
  uint16_t i = 0;

  uint32_t tempUsedSize = 0;
  uint32_t tempCount = 0;
  uint32_t totalBdRamSize = 0;


  if(false == gCheckFirstMalloc)
    {
    // first time
    gCheckFirstMalloc = true;
    }
  else
    {
    ...
    if(EBlockMappingCache[0])
      {
      free(EBlockMappingCache[0]);
      if(EBlockMappingCache)
        {
        free(EBlockMappingCache);
        }
      }
    if(PPAMappingCache[0])
      {
      free(PPAMappingCache[0]);
      if(PPAMappingCache)
        {
        free(PPAMappingCache);
        }
      }
    if(EBMCacheIndex)
      {
  ...
  if(0x0 != totalRamAllowed && EMPTY_DWORD != totalRamAllowed)
    ...
    if(((MAX_PPA_MAP_INDEX * FLUSH_RAM_TABLE_SIZE * NUMBER_OF_DEVICES) + (MAX_PPA_MAP_INDEX * FLUSH_RAM_TABLE_SIZE * NUMBER_OF_DEVICES) + (NUMBER_OF_ERASE_BLOCKS * NUMBER_OF_DEVICES * 2/*ebm entry one size*/)) < tempUsedSize)
      ...
      {
      numBlockMapIndex = (uint16_t) tempCount;
      numPpaMapIndex = (uint16_t) (tempCount * PPA_CACHE_TABLE_OFFSET);
      }
  ...
  else if(EMPTY_DWORD == totalRamAllowed)
    { // Maximum Setting
    numBlockMapIndex = (MAX_EBLOCK_MAP_INDEX);
  ...
  if(thesholdDirtyCount == 0)
    ...
    return FTL_ERR_CACHE_ALLOCTARRAY;
    }

  // Check ram size
  ebmCacheIndexSize = (2/*uint16_t size*/ * numBlockMapIndex * NUMBER_OF_DEVICES);
  eblockMappingCacheSize = (1/*uint8_t size*/ * numBlockMapIndex * NUMBER_OF_DEVICES * FLUSH_RAM_TABLE_SIZE);
  ...
  // EBlockMappingCache
  EBlockMappingCache = (uint8_t **) malloc(sizeof (uint8_t *) * (numBlockMapIndex * NUMBER_OF_DEVICES));
  if(NULL == EBlockMappingCache)
    {
    //DBG_Printf("EBlockMappingCache Malloc Error\n", 0, 0);
    return FTL_ERR_CACHE_ALLOCTARRAY;
    }
  EBlockMappingCache[0] = (uint8_t *) malloc(sizeof (uint8_t) * FLUSH_RAM_TABLE_SIZE * (numBlockMapIndex * NUMBER_OF_DEVICES));
  ...
  // PPAMappingCache
  PPAMappingCache = (uint8_t **) malloc(sizeof (uint8_t *) * (numPpaMapIndex * NUMBER_OF_DEVICES));
  if(NULL == PPAMappingCache)
    {
    //DBG_Printf("PPAMappingCache Malloc Error\n", 0, 0);
    return FTL_ERR_CACHE_ALLOCTARRAY;
    }
  PPAMappingCache[0] = (uint8_t *) malloc(sizeof (uint8_t) * FLUSH_RAM_TABLE_SIZE * (numPpaMapIndex * NUMBER_OF_DEVICES));
  ...
  // EBMCacheIndex
  EBMCacheIndex = (uint16_t *) malloc(sizeof (uint16_t) * (numBlockMapIndex * NUMBER_OF_DEVICES));
...
#endif // #if (CACHE_DYNAMIC_ALLOCATION == true)
...
FTL_STATUS TABLE_TransLogEBInsert(FTL_DEV devID, uint16_t logicalAddr, uint16_t phyEBAddr, uint32_t key)
  ...
  if(TransLogEBArrayCount[devID] >= NUM_TRANSACTLOG_EBLOCKS)
    {
...
FTL_STATUS TABLE_TransLogEBGetNext(FTL_DEV devID, uint16_t * logicalAddrPtr, uint16_t * phyEBAddrPtr, uint32_t * keyPtr)
  ...
  if((TransLogEBArrayCount[devID] > NUM_TRANSACTLOG_EBLOCKS) || (TransLogEBArrayCount[devID] == 0))
    {
...
#if(NUM_TRANSACTLOG_EBLOCKS > 1)
  for(eBlockNum = smallestKeyEB; eBlockNum < (NUM_TRANSACTLOG_EBLOCKS - 1); eBlockNum++)
    {
...
#else  // #if(NUM_TRANSACTLOG_EBLOCKS > 1)
  eBlockNum = 0;
#endif  // #else  // #if(NUM_TRANSACTLOG_EBLOCKS > 1)
...
FTL_STATUS TABLE_TransLogEBGetLatest(FTL_DEV devID, uint16_t * logEBlockPtr,
  ...
  *logEBlockPtr = EMPTY_WORD;
  for(block = 0; block < NUM_TRANSACTLOG_EBLOCKS; block++)
    {
...
FTL_STATUS TABLE_GetTransLogEntry(FTL_DEV devID, uint16_t blockNum, uint16_t * logEBlockPtr, uint16_t * phyAddrPtr, uint32_t * keyPtr)
  {
  if((TransLogEBArrayCount[devID] > NUM_TRANSACTLOG_EBLOCKS) || (TransLogEBArrayCount[devID] == 0))
    {
...
void TABLE_SortTransTable(FTL_DEV devID) /*  1*/
  ...
  last = NUM_TRANSACTLOG_EBLOCKS - 1;
  for(loop = 0; loop < (NUM_TRANSACTLOG_EBLOCKS - 1); loop++)
    {
    end = last;
    last = 0;
    for(count = 0; count < end; count++)
...
#endif  // #if(NUM_TRANSACTLOG_EBLOCKS > 1)
  }
...
FTL_STATUS FTL_FindEmptyTransLogEBlock(FTL_DEV devID, uint16_t * logicalEBNumPtr, uint16_t * physicalEBNumPtr)
  ...
  if((status = TABLE_GetReservedEB(devID, logicalEBNumPtr, false)) != FTL_ERR_PASS)
    {
    return status;
    }
...
FTL_STATUS TABLE_SetTransLogEntry(FTL_DEV devID, uint16_t blockNum, uint16_t logEBlock, uint16_t phyAddr, uint32_t key)
  {
  if((TransLogEBArrayCount[devID] > NUM_TRANSACTLOG_EBLOCKS) || (TransLogEBArrayCount[devID] == 0))
    {
    return FTL_ERR_LOG_SET_ENTRY;
    }
  if(blockNum >= NUM_TRANSACTLOG_EBLOCKS)
    {
...
FTL_STATUS FTL_FindEmptySuperSysEBlock(FTL_DEV devID, uint16_t * logicalEBNumPtr, uint16_t * physicalEBNumPtr)
  ...
  uint32_t eraseCount = 0; /*2*/
  uint16_t ReservedEBNum = 0; /*2*/
  uint32_t lowestErase = EMPTY_DWORD; /*2*/
  uint16_t lowestIndex = EMPTY_WORD; /*2*/

  ReservedEBNum = (uint16_t) (ReservedEBlockNum[devID] & EMPTY_WORD);
  if(ReservedEBNum == 0)
    {
...
FTL_STATUS DataGCForSuperSysEB(void)
  ...
  uint32_t lowestErase = EMPTY_DWORD; /*2*/
  uint16_t lowestEB = EMPTY_WORD; /*2*/
  uint32_t eraseCount = 0; /*2*/
  uint16_t logicalEBNum = 0; /*2*/
...
#if(FTL_DEFECT_MANAGEMENT == true)
    status = InternalForcedGCWithBBManagement(devID, lowestEB, &freedUpPages, &freePageIndex, true);

#else
    status = FTL_InternalForcedGC(devID, lowestEB, &freedUpPages, &freePageIndex, true);
#endif
...
FTL_STATUS ClearSuperEBInfo(void)
  {
  FTL_DEV devID = EMPTY_BYTE; /*1*/
  uint8_t eBlockNum = 0; /*1*/

  for(devID = 0; devID < NUMBER_OF_DEVICES; devID++)
    {
    SuperEBInfo[devID].checkChanged = false;
    SuperEBInfo[devID].checkLost = false;
    SuperEBInfo[devID].checkSuperPF = false;
    SuperEBInfo[devID].checkSysPF = false;
...
FTL_STATUS TABLE_SuperSysEBGetLatest(FTL_DEV devID, uint16_t * logEBlockPtr,
                                     uint16_t * phyEBAddrPtr, uint32_t key)
  {
  uint16_t block = 0; /*2*/
...
FTL_STATUS TABLE_ClearReservedEB(FTL_DEV devID)
  {
  uint16_t numEBlocks = 0; /*2*/ /* changed by Nobu - Feb 17th, 2015 : uint8_t -> uint16_t */
...
FTL_STATUS TABLE_InsertReservedEB(FTL_DEV devID, uint16_t logicalAddr)
  {
  uint16_t numEBlocks = 0; /*2*/ /* changed by Nobu Feb 18, 2015 : uint8_t -> uint16_t for ML16G2 */
  uint16_t findCount = 0; /*2*/ /* changed by Nobu Feb 18, 2015 : uint8_t -> uint16_t for ML16G2 */

#if (FTL_DEFECT_MANAGEMENT == true)
  if(GetBadEBlockStatus(devID, logicalAddr) == true)
    {
    return FTL_ERR_PASS;
    }
#endif
...
FTL_STATUS TABLE_GetReservedEB(FTL_DEV devID, uint16_t * logicalAddrPtr, uint8_t WLflag)
  ...
  // If WLflag = false, the entry with the lowest weight is picked
  // If WLflag = true, the entry with the highest weight is picked
  if(true == WLflag)
    {
    selectedScore = 0;
    }
  for(eblockCount = 0; eblockCount < ReservedEBNum; eblockCount++)
    ...
    logicalEBNum = ReservedEBlock[devID][eblockCount];
#if (FTL_SUPER_SYS_EBLOCK == true)
    if(true == gProtectForSuperSysEBFlag)
      {
...
#if (ENABLE_EB_ERASED_BIT == true)
    if((false == WLflag) && (GetEBErased(devID, logicalEBNum) == true))
      {
...
#endif
    eraseCount = GetTrueEraseCount(devID, logicalEBNum);
    if(((true == WLflag) && (selectedScore < eraseCount)) ||
       ((false == WLflag) && (selectedScore > eraseCount)))
      {
...
FTL_STATUS AdjustFlushEBlockFreePage(FTL_DEV devID, uint16_t logicalFlushEBlockNum, uint16_t dirtyBitMapCount)
  {
  uint16_t sectionNum = 0x0; /*2*/
  uint16_t count = 0x0; /*2*/
  uint16_t incCount = 0x0; /*2*/
...
FTL_STATUS DBG_CheckMappingTables(void)
  ...
  uint16_t eb = 0; /*2*/
  uint16_t eblist[NUM_EBLOCKS_PER_DEVICE];

#if(FTL_EBLOCK_CHAINING == true)
  uint16_t logChainEB = 0; /*2*/
#endif  // #if(FTL_EBLOCK_CHAINING == true)
...
FTL_STATUS DBG_CheckPPAandBitMap(FTL_DEV devID, uint16_t logicalEBNum)
  ...
  uint16_t phyPageOffset2 = EMPTY_WORD; /*2*/
...
FTL_STATUS ClearDeleteInfo(void)
  ...
  for(sectorCount = 0; sectorCount < NUMBER_OF_SECTORS_PER_PAGE; sectorCount++)
    {
    Del_Info.sector[sectorCount] = false;
    }
  return FTL_ERR_PASS;
  }
...
FTL_STATUS InitDeleteInfo(uint8_t devID, uint32_t logicalPageAddr, uint8_t startSector, uint8_t numSectors)
  ...
  for(sectorCount = startSector; sectorCount < (startSector + numSectors); sectorCount++)
    {
    Del_Info.sector[sectorCount] = true;
    }
  return FTL_ERR_PASS;
  }
...
uint8_t GetDeleteInfoNumSectors(void)
  ...
  for(sectorCount = 0; sectorCount < NUMBER_OF_SECTORS_PER_PAGE; sectorCount++)
    {
    if(Del_Info.sector[sectorCount] == true)
      {
      numSectors++;
      }
    }
...
void SetTransferEB(ADDRESS_STRUCT_PTR startPage, ADDRESS_STRUCT_PTR endPage)
  {
  uint16_t logStartEB = EMPTY_WORD; /*2*/
  uint16_t logEndEB = EMPTY_WORD; /*2*/
...
FTL_STATUS FindAndSwapUnusedEB(FTL_DEV devID, uint16_t logFrom, uint16_t * logTo)
  {
  uint8_t flag = false; /*1*/
  uint8_t count = 0; /*1*/
  uint16_t temp = EMPTY_WORD; /*2*/
  uint16_t logicalEBNum = 0; /*2*/
  uint32_t score = EMPTY_DWORD; /*2*/
  uint32_t chosenScore = EMPTY_DWORD; /*2*/
  uint16_t chosenEBNum = EMPTY_WORD; /*2*/
  TRANSFER_EB transferEB = {EMPTY_BYTE, EMPTY_WORD}; /*3*/
  FTL_STATUS status = FTL_ERR_PASS; /*4*/
...
#endif  // #if (true == FTL_EBLOCK_CHAINING)
#if (FTL_DEFECT_MANAGEMENT == true)
    if(GetBadEBlockStatus(devID, logicalEBNum) == true)
      {
...
void SwapUnusedEBlock(FTL_DEV devID, uint16_t logicalDataEB, uint16_t logicalReservedEB)
  {
  uint32_t tempData = EMPTY_WORD; /*2*/
  uint32_t tempRes = EMPTY_WORD; /*2*/
#if (FTL_DEFECT_MANAGEMENT == true)
  uint8_t badEBlockFlag = false;
#endif
  //    uint16_t count = 0;
  ...
  tempRes = (uint16_t) GetPhysicalEBlockAddr(devID, logicalReservedEB);
  tempData = (uint16_t) GetPhysicalEBlockAddr(devID, logicalDataEB);
  SetPhysicalEBlockAddr(devID, logicalReservedEB, (uint16_t) tempData);
  SetPhysicalEBlockAddr(devID, logicalDataEB, (uint16_t) tempRes);
  ...
  SetEBErased(devID, logicalDataEB, false);
  if(true == GetEBErased(devID, logicalReservedEB))
    {
