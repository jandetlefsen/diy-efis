...
#ifdef S29GLxxxN
#include "S29GLxxxN.h"
#define LLD_BUFFER_SIZE (16*LLD_BUF_SIZE_MULTIPLIER)
#define LLD_SUSP_RESUME_DELAY  30
#else
#ifdef S29GLxxxP
#include "S29GLxxxP.h"
...
#include "S26KSxxxS_S26KLxxxS.h"
#define LLD_BUFFER_SIZE (256*LLD_BUF_SIZE_MULTIPLIER)
#define LLD_DPD_DELAY 300
#endif //#ifdef S26KSxxxS_S26KLxxxS
#endif //#ifdef S29XSxxxS
#endif //#ifdef S29WSxxxP
#endif //#ifdef S29WSxxxN 
#endif //#ifdef S29WSxxxR
#endif //#ifdef S29VS064R
#endif //#ifdef S29VSxxxR
#endif //#ifdef S29NSxxxR
#endif //#ifdef S29NSxxxP
#endif //#ifdef S29JLxxxJ
#endif //#ifdef S29GLxxxS
#endif //#ifdef S29GL064S
#endif //#ifdef S29GLxxxP
#endif //#ifdef S29GLxxxN
#endif //#ifdef S29CLxxxJ
#endif //#ifdef S29ALxxxJ
#endif //#ifdef S29ALxxxD

#ifndef LLD_STATUS_REG
FLSTATE FLSTATEOP = FLSTATE_NOT_BUSY;
uint8_t g_suspend_state = FLRESUME;
uint32_t g_suspend_sector, g_writebuffer_sector;
 ...
 * e_errno: 
 */
void lld_GetVersion(uint8_t versionStr[])
  {
  uint8_t* pVer = (uint8_t*) LLD_VERSION;
...
(
 FLASHDATA * base_addr, /* device base address in system */
 uint32_t offset /* sector address offset for ASO(Address Space Overlay) */
 )
  {
...
//add new API for GLT unlockbypass here

/******************************************************************************
 * 
 * lld_UnlockBypassWriteToBuffer - Writes "Unlock Bypass Write to Buffer Pgm" Command sequence to Flash
 *
 * RETURNS: void
 *
 */
void lld_UnlockBypassWriteToBufferCmd
(
 FLASHDATA * base_addr, /* device base address in system */
 uint32_t offset, /* address offset from base address */
 ...
 FLASHDATA *data_buf /* buffer containing data to program */
  ...
  FLASH_WR(base_addr, offset & SA_OFFSET_MASK, NOR_WRITE_BUFFER_LOAD_CMD);
...
/******************************************************************************
 * 
 * lld_UnlockBypassProgramBufferToFlashCmd - Writes "Unlock Bypass Pgm Buffer To Flash" Cmd sequence to Flash
 *
 * RETURNS: void
 * 
 */
void lld_UnlockBypassProgramBufferToFlashCmd
(
 FLASHDATA * base_addr, /* device base address in system */
 uint32_t offset /* address offset from base address */
 )
  {
  /* Transfer Buffer to Flash Command */
  FLASH_WR(base_addr, (offset & SA_OFFSET_MASK), NOR_WRITE_BUFFER_PGM_CONFIRM_CMD);
  }

/******************************************************************************
 * 
 * lld_UnlockBypassWriteToBufferAbortResetCmd - Writes "Unlock Bypass Write To Buffer Abort" Reset to Flash
 *
 * RETURNS: void
 *
 */
void lld_UnlockBypassWriteToBufferAbortResetCmd
(
 FLASHDATA * base_addr /* device base address in system */
 )
  {
...
/******************************************************************************
 * 
 * lld_UnlockBypassSectorEraseCmd - Writes "Unlock Bypass Sector Erase" Cmd sequence to Flash
 *
 * RETURNS: void
 *
 */
void lld_UnlockBypassSectorEraseCmd
(
 FLASHDATA * base_addr, /* device base address in system */
 uint32_t offset /* address offset from base address */
 )
  ...
  FLASH_WR(base_addr, offset & SA_OFFSET_MASK, NOR_SECTOR_ERASE_CMD);
  }

/******************************************************************************
 * 
 * lld_UnlockBypassChipEraseCmd - Writes "Unlock Bypass Chip Erase" Cmd sequence to Flash
 *
 * RETURNS: void
 *
 */
void lld_UnlockBypassChipEraseCmd
(
 FLASHDATA * base_addr /* device base address in system */
 )
  {
  FLASH_WR(base_addr, 0, NOR_ERASE_SETUP_CMD);
  FLASH_WR(base_addr, 0, NOR_CHIP_ERASE_CMD);
  }
...
#ifndef LLD_CONFIGURATX8X16_AS_X8
#ifndef LLD_CONFIGURATX8_AS_X8
...
(
 FLASHDATA * base_addr, /* device base address in system */
 uint32_t offset /* ASO(Address Space Overlay) */
 )
  {
  unsigned int id;
...
/******************************************************************************
 * 
 * lld_GetDeviceId - Get device ID operation (for RPC2)
 *
 * RETURNS: three byte ID in a single int
 *
 */
...
(
 FLASHDATA * base_addr, /* device base address in system */
 uint32_t offset /* sector offset for ASO(Address Space Overlay) */
 )
  {
  /* Issue SecSi Sector Entry Command Sequence */
...
(
 ...
 FLASHDATA *data_buf /* buffer containing data to program */
   ...
   */
#ifdef LLD_WRITE_BUFFER_OP_1 //RPC2 device doesn't ignore lower address bits(a0-a15)
  FLASH_WR(base_addr, offset & SA_OFFSET_MASK, wcount);
#else
...
/******************************************************************************
 * 
 * lld_UnlockBypassBufferWriteProgramOp - Performs an Unlock Bypass Write Buffer Programming Operation.
 *
 * Function programs a write-buffer overlay of addresses to data 
 * passed via <data_buf>.
 * Function issues all required commands and polls for completion.
...
DEVSTATUS lld_UnlockBypassBufferWriteProgramOp
(
 FLASHDATA * base_addr, /* device base address in system     */
 uint32_t offset, /* address offset from base address  */
 uint32_t word_count, /* number of words to program        */
 FLASHDATA *data_buf /* buffer containing data to program */
  ...
  /* Issue Program Buffer to Flash command */
  lld_UnlockBypassProgramBufferToFlashCmd(base_addr, (offset & SA_OFFSET_MASK));

  status_reg = lld_Poll(base_addr, (offset + word_count - 1));
...
(
 ...
 FLASHDATA *data_buf /* buffer containing data to program */
  ...
  status = lld_Poll(base_addr, (offset + word_count - 1), &data_buf,
                    &read_data, LLD_P_POLL_WRT_BUF_PGM);
  return (status);
  }
#endif /* LLD_STATUS_REG */
#endif /* #if defined(LLD_WRITE_BUFFER_OP)  */
...
/******************************************************************************
 * 
 * lld_UnlockBypassProgramOp - Performs an Unlock Bypass Programming Operation.
 *
 * Function programs a single location to the specified data.
 * Function issues all required commands and polls for completion.
 *
...
DEVSTATUS lld_UnlockBypassProgramOp
(
 FLASHDATA * base_addr, /* device base address is system */
 uint32_t offset, /* address offset from base address */
 FLASHDATA write_data /* variable containing data to program */
  ...
  {
  DEVSTATUS status;
  status = lld_UnlockBypassBufferWriteProgramOp(base_addr, offset, 1, &write_data);
  return (status);
  }
#else
...
(
 ...
 FLASHDATA write_data /* variable containing data to program */
  ...
  lld_UnlockBypassProgramCmd(base_addr, offset, &write_data);
  status = lld_Poll(base_addr, offset, &write_data, &read_data, LLD_P_POLL_PGM);
  return (status);
  }
#endif
#endif /* #if defined(LLD_PROGRAM_OP) */
...
/******************************************************************************
 * 
 * lld_UnlockBypassSectorEraseOp - Performs a Unlock Bypass Sector Erase Operation
 *
 * Function erases the sector containing <base_addr> + <offset>.
 * Function issues all required commands and polls for completion.
 *
...
DEVSTATUS lld_UnlockBypassSectorEraseOp
(
 FLASHDATA * base_addr, /* device base address is system */
 uint32_t offset /* address offset from base address */
 )
...
#endif /* #if defined (LLD_SECTOR_ERASE_OP) && defined (S29GLxxT) */
...
/******************************************************************************
 * 
 * lld_UnlockBypassChipEraseOp - Performs a Unlock Bypass Chip Erase Operation
 *
 * Function erases entire device located at <base_addr>.
 * Function issues all required commands and polls for completion.
 *
...
DEVSTATUS lld_UnlockBypassChipEraseOp
(
 FLASHDATA * base_addr /* device base address in system */
 )
  {
...
#endif /* #ifdef LLD_STATUS_REG */
#endif /* #if defined(LLD_CHIP_ERASE_OP) */
...
(
 FLASHDATA * base_addr, /* device base address in system */
 uint32_t offset /* Sector offset address */
 )
  {
  FLASH_WR(base_addr, 0, NOR_UNLOCK_BYPASS_PROGRAM_CMD);
 ...
 *
 */
int lld_PpbProgramOp(FLASHDATA *base_addr, uint32_t offset)
  {
  DEVSTATUS dev_status = DEV_STATUS_UNKNOWN;
#ifdef LLD_STATUS_GET
#ifndef LLD_POLL_STATUS
...
FLASHDATA lld_PpbStatusReadOp(FLASHDATA *base_addr, uint32_t offset)
  {
  FLASHDATA status;
 ...
 *
 */
FLASHDATA lld_SSRLockRegBitsReadOp(FLASHDATA *base_addr, uint32_t offset)
  {
  FLASHDATA value;
 ...
 *
 */
int lld_SSRLockRegBitsProgramOp(FLASHDATA *base_addr, uint32_t offset, FLASHDATA value)
  {
  FLASHDATA status_reg;
...
void lld_SectorLockCmd(
  ...
  // Lock command
  FLASH_WR(base_addr, 0, NOR_SECTOR_LOCK_CMD); //LSA = 0, lock all sectos
  }
...
int lld_SectorLockRangeCmd(
                           FLASHDATA *base_addr,
                           uint32_t StartSec, /* first sector in range */
                           uint32_t StopSec /* last sector in range */
                           )
  {
...
/******************************************************************************
 * 
 * lld_EvaluateEraseStatusCmd - Evaluate Erase Status
 * Note: n/a
 * RETURNS: n/a
 *
 */
void lld_EvaluateEraseStatusCmd
(
 FLASHDATA *base_addr,
 uint32_t offset
 )
...
/******************************************************************************
 * 
 * lld_ContinuityCheckOp - Continuity Check
 * Note: n/a
 * RETURNS: DEVSTATUS
 *
 */
DEVSTATUS lld_ContinuityCheckOp
(
 FLASHDATA *base_addr
 )
  {
  ...
  //Set up
  FLASH_WR(base_addr, LLD_UNLOCK_ADDR1, NOR_STATUS_REG_CLEAR_CMD); //Clear die zero status
  FLASH_WR(base_addr, LLD_UNLOCK_ADDR1, NOR_STATUS_REG_READ_CMD); //Write Status Register Read command to die sero
  status_reg = (FLASH_RD(base_addr, 0)); //Read status from die zero to confirm status bit zero = 0
  if((status_reg & DEV_CONTINUITY_CHECK_MASK) == DEV_CONTINUITY_CHECK_MASK)
    return (DEV_CONTINUITY_CHECK_PATTERN_ERROR);

  //Continuity Pattern
  FLASH_WR(base_addr, LLD_CONTINUITY_CHECK_ADDR_1, LLD_CONTINUITY_CHECK_DATA_1); //First continuity cycle
  FLASH_WR(base_addr, LLD_CONTINUITY_CHECK_ADDR_2, LLD_CONTINUITY_CHECK_DATA_2); //Second continuity cycle

  //Verify Continuity Pattern Detected
  FLASH_WR(base_addr, LLD_UNLOCK_ADDR1, NOR_STATUS_REG_READ_CMD); //Write Status Register Read command to die zero
  status_reg = (FLASH_RD(base_addr, 0)); //Read Status from die zero to contirm status bit zero = 1 for continuity pattern detected
  if((status_reg & DEV_CONTINUITY_CHECK_MASK) != DEV_CONTINUITY_CHECK_MASK)
    return (DEV_CONTINUITY_CHECK_NO_PATTERN_ERROR);
  else
    return (DEV_CONTINUITY_CHECK_PATTERN_DETECTED);
  }
#endif /* LLD_CONTINUITY_CHECK_CMD */
#endif /* #ifdef S29GL064S */
